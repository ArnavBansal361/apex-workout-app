...
export function PerformanceInsightsCard({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`apex-card p-5 ${className}`.trim()}>
      ...
      {!eligible ? (
        <>
          <p className="text-[12px] font-medium text-[var(--apex-text-secondary)] mb-3 leading-relaxed">
        ...
        <>
          <p className="text-[12px] font-medium text-[var(--apex-text-secondary)] mb-4 leading-relaxed">
          ...
          {insights.length > 0 ? (
            <ul className="space-y-3">
              {insights.map((insight) => (
                <li
                  key={insight.id}
                  className="text-[13px] font-medium text-[var(--apex-text-primary)] leading-relaxed pl-3 border-l-2 border-[var(--apex-accent)]"
                >
                  {insight.text}
                </li>
              ))}
